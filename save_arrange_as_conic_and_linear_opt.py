...
#             task.putqobj(qsubi, qsubj, qval)  #  最小化向量模长
            # 添加conic constraints
            conic_val = [i for i in range(numvar)]
            random.shuffle(conic_val)
...
#             print('开始放入线性约束')
            if if_quite:
                iterater = range(len(bkc))
            else:
                iterater = tqdm(range(len(bkc)))
            for i in iterater:
                con_idx = i
...
def greedy_embed(sym_mat, all_embedding, start_emb_num, vocab_size):
    ...
    for current_word_idx in range(start_emb_num, vocab_size):
        ...
        while not_linear_independent: #  or solsta == mosek.solsta.prim_infeas_cer
            ...
            # 检查是否位于
            # 因为不明原因导致即使可行域不空时，问题依然有可能无解，此时用可行域内的任意一点做替代
            repeat_time = 0
            if_change_lint = False
            while solsta != mosek.solsta.optimal: 
                if if_infeasible:
                    bkc, blc, buc, asub, aval, if_infeasible, \
                    new_bound_idx_list, new_bound_val_list= check_feasibility(bkc, blc, buc, asub, aval, 
                                                                  bound_idx_list, bound_val_list, bound_A_list, 
                                                                  ele_order_list, all_embedding, emb_dim)
                repeat_time += 1
                min_quad_vec, solsta = solve_min_quad(bkc, blc, buc, asub, aval, qsubi, qsubj, qval, emb_dim, True)
                print(f'重复计算了当前嵌入{repeat_time}次。共{cmb}种可能。',end='\r')
                if_change_lint = True
                if repeat_time > cmb:
