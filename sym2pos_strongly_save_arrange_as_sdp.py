...
def get_ele_order(sym_mat):
    ...
    ele_order_list = np.zeros([num_ele, 4])
    ele_idx_in_list = 0;
...
#             print(f'row:{row_i};col_j:{col_j}', end='\r')
    ...
    for i in range(ele_order_list.shape[0]-1):
        ele_order_list[i, 3] = ele_order_list[i+1, 2] - ele_order_list[i, 2]
...
#     for xi_id in tqdm(range(ele_num)):
    for xi_id in (range(ele_num)):
        ...
        # blx.append(0)
        blx.append(ele_order_matrix[xi_id,3])
        bux.append(+inf)
        asub.append([xi_id])
        aval.append([-1])
...
#     for con_id in tqdm(range(ele_num)):
        ...
        if con_id < ele_num-1:  # 最大元素需要区别对待
            ...
            [small_ele_row, small_ele_col, small_ele_val] = ele_order_matrix[con_id, :3]
            [biger_ele_row, biger_ele_col, biger_ele_val] = ele_order_matrix[con_id+1, :3]
